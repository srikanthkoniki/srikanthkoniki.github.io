...
function googleConfigured() {
  return (
    ...
    !window.PLANNER_CONFIG.CLIENT_ID.includes("PASTE_YOUR_GOOGLE_CLIENT_ID_HERE")
  );
}
      ...
      try {
        ...
        tokenClient =
          google.accounts.oauth2.initTokenClient({
            client_id: PLANNER_CONFIG.CLIENT_ID,
            scope: SCOPES,
            ...
            callback: async function(response) {
              ...
              try {
                ...
                console.error(error);
                $("loginMessage").textContent =
                  "Could not open your planner. Check Google permissions.";
              }
            }
          });
...
async function loadPlannerData() {
  ...
  rows.slice(1).forEach(function(row) {
    ...
    if (type === "month" && field === "plan") {
      plannerData.months[key] = value;
    }
  });
}

async function saveRow(type, key, field, value) {
